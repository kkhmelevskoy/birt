...
public class Transformer
{
			...
			Object data, Style style )
	{
		...
		{
			if ( sheet.addMergeBlock( coords.y1,
					coords.x1,
					coords.y2,
					coords.x2 ).isEmpty( ) )
			{
	...
	private void layoutChildren( List<Integer> xCuts, List<Integer> yCuts,
			...
			Style currentStyle, int xOffset, int yOffset )
	{
		Frame element = null;
		...
		for ( Iterator<Frame> it = parentElement.iterator( ); it.hasNext( ); )
		{
			...
			style = LocalStyle.create( new Style[]{
			...
			else
			{
				...
				if ( coord.x1 == coord.x2 && coord.y1 == coord.y2 )
				{
					...
					if ( cell != defaultCell )
					{
						System.out.println( "[WARNING]:An existing single cell detected" ); //$NON-NLS-1$
					}
				...
				{
					// Try merge first.
					if ( sheet.addMergeBlock( coord.y1,
							coord.x1,
							coord.y2,
							coord.x2 ).isEmpty( ) )
					{
