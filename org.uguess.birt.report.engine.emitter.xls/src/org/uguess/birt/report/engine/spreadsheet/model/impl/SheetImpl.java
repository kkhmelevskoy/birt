...
public class SheetImpl extends BlockImpl implements Sheet
{
	...
	private int getActiveRange( boolean isRow )
	{
		...
		Iterator<Integer> itr = isRow ? rows.keySet( ).iterator( )
				: cols.keySet( ).iterator( );
		while ( itr.hasNext( ) )
		{
			int v = itr.next( );
			max = Math.max( max, v );
			...
			MergeBlock mergeValue )
	{
		...
		{
			for ( int j = mergeRange.getStartColumn( ); j <= mergeRange.getEndColumn( ); j++ )
			{
				setCell( i, j, cellPool.getCell( getCell( i, j, create ),
						mergeValue ), false );
			}
		}
	}
